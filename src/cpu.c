...
void execute_lr_w(CPU *cpu, uint32_t rd, uint32_t rs1) {
    uint64_t address = cpu->registers[rs1];
    uint64_t value = memory_load_word(cpu->memory, address);
    cpu->registers[rd] = value;
    cpu->reserved_address = address;
}

void execute_sc_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t value = cpu->registers[rs2];
    if (cpu->reserved_address == address) {
        memory_store_word(cpu->memory, address, value);
...
void execute_amoswap_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    uint64_t mem_value = memory_load_word(cpu->memory, address);
    memory_store_word(cpu->memory, address, src_value);
    cpu->registers[rd] = mem_value;
}

void execute_amoadd_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    // 从内存地址加载值
    uint64_t mem_value = memory_load_word(cpu->memory, address);
    ...
    // 将结果存储回内存地址
    memory_store_word(cpu->memory, address, result);
    // 将内存中的旧值返回到目标寄存器
    cpu->registers[rd] = mem_value;
}

void execute_amoand_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    // 从内存地址加载值
    uint64_t mem_value = memory_load_word(cpu->memory, address);
...
void execute_amoxor_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    // 从内存地址加载值
    uint64_t mem_value = memory_load_word(cpu->memory, address);
...
void execute_amoor_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    // 从内存地址加载值
    uint64_t mem_value = memory_load_word(cpu->memory, address);
...
void execute_amomin_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    int64_t src_value = (int64_t)cpu->registers[rs2];
    // 从内存地址加载值
    int64_t mem_value = (int64_t)memory_load_word(cpu->memory, address);
    // 执行有符号比较并选择较小的值
    int64_t result = (src_value < mem_value) ? src_value : mem_value;
    // 将结果存储回内存地址
    memory_store_word(cpu->memory, address, (uint64_t)result);
    // 将内存中的旧值返回到目标寄存器
    cpu->registers[rd] = (uint64_t)mem_value;
}


void execute_amomax_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    int64_t src_value = (int64_t)cpu->registers[rs2];
    // 从内存地址加载值
    int64_t mem_value = (int64_t)memory_load_word(cpu->memory, address);
...
void execute_amominu_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    // 从内存地址加载值
    uint64_t mem_value = memory_load_word(cpu->memory, address);
...
void execute_amomaxu_w(CPU *cpu, uint32_t rd, uint32_t rs1, uint32_t rs2) {
    uint64_t address = cpu->registers[rs1];
    uint64_t src_value = cpu->registers[rs2];
    // 从内存地址加载值
    uint64_t mem_value = memory_load_word(cpu->memory, address);
